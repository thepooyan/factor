                      ...
                      <div class="p-4 pt-0 space-y-2 bg-gray-50/30">
                        <For each={feature.addons.filter((a) => a.is_active)}>
                          {(addon) => {
                            ...
                            return (
                              <div
                                class={`p-2 bg-white rounded-xl border flex items-center justify-between transition-all ${
                                  isDisabled()
                                    ? "opacity-40 grayscale cursor-not-allowed"
                                    : "hover:border-blue-200 cursor-default"
                                }`}
                              >
                                <Show when={addon.discounted_price && addon.discounted_price !== 0}>
                                  <span class="text-[10px] bg-red-100 text-red-600 px-1.5 rounded font-bold absolute top-[-8px] right-2 rotate-12 z-10 border border-red-200 shadow-sm">
                                    {Math.round(((addon.price - addon.discounted_price) / addon.price) * 100)}%
                                  </span>
                                </Show>
