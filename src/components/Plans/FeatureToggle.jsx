...
export function FeatureToggle(props) {
  ...
  const isFeatureIncludedInPlan = (feature) => {
    ...
    return plans.some(planName => feature.for_plans && feature.for_plans[planName] === true);
  };
          ...
          <div class="flex flex-col gap-4">
            <For each={sortedFeatures().filter((_, i) => i % 2 === columnIndex)}>
              {(feature) => {
                ...
                const isAnyAddonInCart = () => feature.addons.some(a => cart.isItemInCart(`addon-${a.addon_id}`));

                return (
                  <div class={`bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden transition-all h-fit ${isIncluded() ? 'opacity-70 grayscale-[0.5]' : ''}`}>
                    <button
                      onClick={() => setIsOpen(!isOpen())}
                      class={`w-full p-3 bg-white hover:bg-gray-50 flex flex-col sm:flex-row items-center justify-between ${isIncluded() ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                      <div class="flex flex-col items-center sm:items-start text-center sm:text-right">
                        <h4 class="font-bold text-gray-900 text-lg flex items-center">
                          ...
                          <Show when={isIncluded()}>
                            <span class="mr-2 text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">موجود در پلن شما</span>
                          </Show>
                        </h4>
                        <p class="text-gray-500 text-xs mt-1 line-clamp-1">{feature.description}</p>
                        <Show when={!isIncluded()}>
                           <div class="text-sm font-bold text-indigo-600 mt-1">از {formatPriceJS(feature.addons[0].price)} تومان</div>
                        </Show>
                      </div>
                      ...
                      <div class="p-4 pt-0 space-y-2 bg-gray-50/30">
                        <For each={feature.addons.filter((a) => a.is_active)}>
                          {(addon) => {
                            ...
                            const isDisabled = () => isIncluded() || (isAnyAddonInCart() && !isInCart());

                            return (
                              <div class={`p-2 bg-white rounded-xl border flex items-center justify-between transition-all ${isDisabled() ? 'opacity-40 grayscale cursor-not-allowed' : 'hover:border-blue-200 cursor-default'}`}>
                                ...
                                <div class="flex items-center gap-3">
                                  ...
                                  <button
                                    ...
                                    disabled={isDisabled()}
                                    class={`flex items-center justify-center p-2 rounded-lg text-white transition-colors ${isInCart() ? "bg-red-500" : "bg-blue-600"} ${isDisabled() ? 'cursor-not-allowed' : 'cursor-pointer hover:bg-opacity-90'}`}
                                  >
